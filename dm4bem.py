...
def tc2ss(TC):
    ...
    # indexes of inputs
    idx_G = pd.Series(np.arange(TC['G'].shape[0]))
    idx_C = pd.Series(np.arange(TC['C'].shape[0]))
    idx_u = pd.concat([idx_G, idx_C])

    G = pd.DataFrame(np.diag(G), index=np.arange(TC['G'].shape[0]), columns=np.arange(TC['G'].shape[0]))
    C = pd.DataFrame(np.diag(C), index=np.arange(TC['C'].shape[0]), columns=np.arange(TC['C'].shape[0]))
    ...
    try :
        ...
        pass  # ignore complètement toute erreur
